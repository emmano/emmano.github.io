...
1. When our test runs, our test obeject gets injected twice. Once by the production code per-se. The second time, our test object is injected by `DaggerTestComponent` inside our test. This means that if we need to test an `Activity` that calls its `onCreate()` twice on the same test, we would have to re-inject the test object.
